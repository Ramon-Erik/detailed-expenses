export const formRules = {
  ...
  validPhoneNumber: function (value: string): boolean | string {
    return (!!value && /^\(\d{2}\) \d{5}-\d{4}$/.test(value)) || 'Insira um número válido'
  },
  ...
  validCNPJ: function (value: string): boolean | string {
    ...
    if (!/^\d{14}$/.test(numericCNPJ)) return 'CNPJ inválido. Deve conter 14 dígitos.'
    return true
  },

  matches: function (value: string, errorMessage?: string): (value: string) => boolean | string {
    return function (str: string) {
      if (!str) {
        return true
      }
  ...
  validTelefone: function (value: string): boolean | string {
    ...
    if (!/^\d{11}$/.test(numericTelefone)) return 'Telefone inválido. Deve conter 11 dígitos.'
    return true
  },

  validCEP: function (errorMessage?: string): (value: string) => boolean | string {
    return function (str: string) {
      if (!str) {
        return true
      }
  ...
  validTotalValue: function (errorMessage?: string): (value: string) => boolean | string {
    return function (str: string) {
      if (!str) {
        return true
      }
  ...
  hasToBeEqual(value: string | boolean | number) {
    return (val: string | boolean | number) =>
      val === value ? true : 'os valores não estão iguais'
  },
}
